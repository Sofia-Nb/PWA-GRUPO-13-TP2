# Gestor de Tanques PWA-GRUPO-13-TP2

## Objetivo

El objetivo del proyecto es desarrollar una SPA (Single Page Application) utilizando React y Tailwind. La aplicación consumirá datos desde una API simulada e incluirá navegación entre distintas páginas, sistema de búsqueda, soporte para idiomas en español e inglés, y persistencia de las preferencias del usuario.

---

## Integrantes del Grupo
* **Sastre Juan Ignacio** - FAI-4491
* **Gonzalez Marcos Nahuel** - FAI-4869
* **Bascur Sofia Natali** - FAI-4306

---

## Descripción de la Aplicación

La aplicación ofrece funcionalidades de:

* **Visualización:** Listado dinámico de cada tipo de Tanque con tarjetas detalladas.
* **Filtros:** Filtros de busqueda según la preferencia del usuario
* **Busqueda:** Sistema de búsqueda para encontrar Tanques específicos de manera rápida.
* **Gestion de Favoritos:** El usuario puede marcar y desmarcar Tanques como favoritos para acceder a ellos fácilmente.
* **Internacionalización:** Soporte de idiomas (español e inglés) con almacenamiento de preferencias del usuario.
* **Diseño responsive:** La interfaz se adapta a distintos tamaños de pantalla (móviles, tablets y escritorio), brindando una experiencia de usuario consistente en cualquier dispositivo.

---

## Guia de Instalacion y Ejecucion

Segui estos pasos para tener el proyecto funcionando localmente:

### 1. Clonar el repositorio
Abrir una terminal y ejecutar:
```bash
git clone https://github.com/Sofia-Nb/PWA-GRUPO-13-TP2.git
cd PWA-GRUPO-13-TP2
```

### 2. Instalar las dependencias
```bash
    npm install
```

### 3. Correr la aplicación
```bash
    npm run dev
 ```

## Tecnologias usadas

- React
- React Router DOM
- Tailwind CSS
- i18next